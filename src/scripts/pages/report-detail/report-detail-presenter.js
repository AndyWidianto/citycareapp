...
export default class ReportDetailPresenter {
  ...
  async saveReport() {
    try {
      ...
      this.#view.saveToBookmarkSuccessfully('Succes to save to bookmark');
    } catch (err) {
      console.error('saveReport: error:', err);
      this.#view.saveToBookmarkFailed(err.message);
    }
  }
  ...
  async removeReport() {
    try {
      ...
      console.error('removeReport: error:', err);
      this.#view.removeFromBookmarkFailed(err.message);
    }
  }
